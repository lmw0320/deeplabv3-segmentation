...
def find_last(log_path): #使用最新模型文件来进行检测
    checkpoints = next(os.walk(log_path))[2]
    checkpoints = filter(lambda f: f.endswith('h5'), checkpoints)
    checkpoints = sorted(checkpoints)
    if not checkpoints:
        raise FileNotFoundError('请事先训练模型，再进行检测。')
    model_path = os.path.join(log_path, checkpoints[-1])
    return model_path

def focal_loss(logits, targets, alpha = 0.25, gamma = 2, normalizer = 1.0):
